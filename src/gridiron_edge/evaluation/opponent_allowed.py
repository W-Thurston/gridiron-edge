...
Cohorts:
    - season: all games in the latest NFL_YEAR
    - l5: last 5 games rolling (defense's most recent 5 games)
...
COHORTS: Final[list[str]] = ["season", "l5"]


def compute_opponent_allowed(player_game_logs: DataFrame) -> DataFrame:
    """Compute per-defense per-position stat aggregations.

    For each (opponent_team, position, stat_type) combination, computes
    two cohort snapshots:
        - season: average across all games in the current season
        - l5: average across defense's last 5 games (season-ordered)

    Then ranks within (position, stat_type, cohort) — 1 = stingiest.

    Args:
        player_game_logs: DataFrame with columns player_id, team,
            opponent_team, position, season, week, game_id, and stat
            columns (passing_yards, rushing_yards, receiving_yards).

    Returns:
        DataFrame with columns: opponent_team, position, stat_type,
        cohort, avg_allowed, sample_size, rank_against_position.
        Empty if input is empty or missing required columns.
    """
    ...
    # Filter to the latest season only.
    ...
    for stat_type, stat_col in STAT_COLUMN_MAP.items():
        ...
        # For each stat_type, filter players by position, then
        # sum per (opponent_team, game_id) and average across games.
        stat_data = scope.loc[scope["position"] == position, :]
        if stat_data.empty:
            continue

        # Sum per game: for each game, how much stat did all players
        # of `position` on the offense produce?
        per_game = (
            stat_data.groupby(["opponent_team", "game_id", "week"])[stat_col].sum().reset_index()
        )

        # Season cohort: average across all games.
        season_rows = _compute_cohort(
            per_game,
            stat_col=stat_col,
            cohort="season",
            stat_type=stat_type,
        ...
        all_rows.append(season_rows)

        # L5 cohort: for each defense, take last 5 games.
        l5_input = (
            per_game.sort_values(["opponent_team", "week"])
            .groupby("opponent_team", group_keys=False)
            .tail(5)
        )
        l5_rows = _compute_cohort(
            l5_input,
            stat_col=stat_col,
            cohort="l5",
            stat_type=stat_type,
...
def _compute_cohort(
    ...
    *,
    ...
    Args:
        ...
        cohort: 'season' or 'l5' — for output labeling.
