...
def _reconstruct_away_home(games: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    away_won = games["GAME_LOCATION"] == _AWAY_WIN_LOCATION
    games = games.copy()
...
def backfill_model(
    ...
    *,
    ...
    df_new = predictor.predict_historical(games, repo=resolved_repo)
    ...
    if not overwrite:
        existing = load_prediction_log(model_version=model_version, repo=resolved_repo)
        if not existing.empty:
            already_archived = set(existing["game_id"].unique())
