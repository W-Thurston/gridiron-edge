...
| player_game_logs `game_id` misaligned with player rows | player-log ingest (nflreadpy build) | Per-game `game_id` points at unrelated games (e.g., A.Barner SEA week 1 shows game_id "2025_01_ARI_NO"). `week`, `value`, `opponent_team` appear correct; `game_id` is scrambled — likely a join/assignment bug in the player-log build. Breaks any is_home derived from game_id. Surfaced building the B1 player-history endpoint (W9.10); worked around by dropping game_id/is_home from the API response. Fix upstream so game_id realigns; would restore trustworthy is_home. |
