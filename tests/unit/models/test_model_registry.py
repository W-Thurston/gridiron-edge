...
from gridiron_edge.models.base import GameModel, Model
...
class TestModelRegistryGet:
    ...
    def test_unknown_model_raises_key_error(self) -> None:
        with pytest.raises(KeyError, match="not_a_model"):
            ModelRegistry.get("not_a_model")

    def test_all_returns_dict(self) -> None:
        all_models: dict[str, type[Model]] = ModelRegistry.all()
        assert isinstance(all_models, dict)

    def test_all_keys_are_strings(self) -> None:
        for name in ModelRegistry.all():
    ...
    def test_get_roundtrips_with_all(self) -> None:
        ...
        for name in ModelRegistry.all():
            cls: type[Model] = ModelRegistry.get(name)
